...
namespace area452.Controllers
{
    public class TesteController : Controller
    {
        ...
        public JsonResult GetLocs()
        {
            ...
            return Json(dados.Take(5), JsonRequestBehavior.AllowGet);
   

        }
        /// <summary>
        /// distancai entre duas latitudes
        /// </summary>
        ...
        public double getDistance(double lat1, double lon1,double lat2, double lon2) 
        {
            ...
            var distance = 3963.0 * Math.Acos(Math.Sin(lat1 * Math.PI / 180) * Math.Sin(lat2 * Math.PI / 180) + Math.Cos(lat1* Math.PI/180)* Math.Cos(lat2* Math.PI/180) * Math.Cos( (lon2*Math.PI/180) - (lon1*Math.PI/180) ));
            return distance;
